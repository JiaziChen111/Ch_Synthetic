...
foreach group in 0 1 {
	...
	foreach t in 24 120 {
		...
		foreach v in `vars' {
			...
			capture {
			foreach shock in path { // mp1 path lsap {
			...
			// controls
			local ctrl`v'`t'm l(1/`maxlag').`v'`t'm l(1/1).fx
// 			local ctrl`v'`t'm l(1/`maxlag').d`v'`t'm l(1/`maxlag').fx
			
			forvalues h = 0/$horizon {
				// response variables
				capture gen `v'`t'm`h' = (f`h'.`v'`t'm)
...
// 				graph export $pathfigs/LPs/`shk'/`grp'/`v'`t'm.eps, replace
			...
			graph export $pathfigs/LPs/`shk'/`grp'/`shk'`grp'nomyptpphi`t'mPost.eps, replace
			graph drop _all
		}		// `shock'
	}		// `t' tenor
}		// `group' AE or EM
