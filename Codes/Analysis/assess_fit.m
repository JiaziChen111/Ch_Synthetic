...
fitrprt(2:2:2*length(currEM)+1,1) = currEM;                                 % names of EMs
fitrprt(2*length(currEM)+2:2:2*ncntrs+1,1) = currAE;                        % names of AEs
...
% Report MAE only
% fitrprt = cell(ncntrs+1,length(matsall)+1);
% fitrprt(1,2:end) = num2cell(matsall);
% fitrprt(2:length(currEM)+1) = currEM;
% fitrprt(length(currEM)+2:ncntrs+1) = currAE;
% fitrprt(2:end,2:end) = num2cell(mtxmae);