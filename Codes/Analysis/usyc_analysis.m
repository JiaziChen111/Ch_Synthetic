...
% Estimate parameters via JSZ normalization (use yields only: ylds)
...
tpyldjsz  = (ylds_Qjsz - ylds_Pjsz)*100;                                    % in percent
...
while exitflag == 0
    if niter == 2000
        ...
        par0  = [PhiP(:);cSgm(:);lmbd1(:);lmbd0(:);mu_xP(:);rho1(:);rho0;sgmY]; % fixed sgmS case
...
tpyldsvy  = (ylds_Q - ylds_P)*100;                              % = ones(nobs,1)*(AnQ - AnP) + xs'*(BnQ - BnP);

% Figures
figure; plot(dates,cP_filtered(:,1),dates,xs0(1,:))
