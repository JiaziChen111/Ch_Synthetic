...
y = TTycsv{:,:}./100;                                                           % ylds as decimals
ylds = y(:,1:8);
svys = y(:,9:end);
[mean(ylds,'omitnan'); std(ylds,'omitnan')]                                     % table 1a
plot(ylds)                                                                      % figure 1a
plot(svys,'*')                                                                  % figure 1b
...
rho0  = rho0_cP*dt; lmbd0 = Sgm\(mu_xP - mu_xQ);
rho1  = rho1_cP*dt; lmbd1 = Sgm\(PhiP - PhiQ);
...
optns  = optimset('MaxFunEvals',length(par0)*500,'MaxIter',length(par0)*500);
