function S = daily2monthly(dataset_daily,header_daily,S,YCtype)
% This function constructs panels of monthly local currency (LC) yield
% curves (YCs) for the countries in the structure S.
%
%     INPUT
...
% char:   YCtype        - type of LC yield curve to extract, i.e. nominal (LC) or synthetic (LCSYNT)
% struct: S             - names of countries and currencies, letter and digit codes
%
%     OUTPUT
% struct: S - includes date of first obs, panels of LC yield curves, evolution of tenors available
%
%   ASSUMPTIONS
...
%%
% LC type, tenors and field names
...
if strcmp(YCtype,'LC'); prefix = 'nom'; elseif strcmp(YCtype,'LCSYNT'); prefix = 'syn'; end
fnames   = strcat(prefix,fnames);

% Construct the database of LC yield curves
for k  = 1:ncntrs
    % Available tenors per country
    fltrYLD = ismember(header_daily(:,1),S(k).iso) & fltrLC;            % Country + LC data
    if strcmp(YCtype,'LC') && any(fltrYLD & fltrYCs)                    % Countries with 2 nominal LC YCs
        fltrYLD = fltrYLD & fltrYCs;                                    % Use BFV LC YC
    end
    tnrs    = tnrs_all(fltrYLD);                                        % Tenors available
    
    % End-of-month data
    idxDates   = sum(~isnan(dataset_daily(:,fltrYLD)),2) > 4;           % Rows with at least 5 observations
    fltrYLD(1) = true;                                                  % To include dates
    data_lc    = dataset_daily(idxDates,fltrYLD);                       % LC data with at least 5 observations
    data_lc    = end_of_month(data_lc);                                 % Keep end-of-month observations
    S(k).(fnames{1}) = datestr(data_lc(1,1),'mmm-yyyy');                % First monthly observation
    S(k).(fnames{2}) = [nan tnrs'; data_lc(:,1) data_lc(:,2:end)/100];	% Yields as decimals
    
    % Tenors with actual data fluctuate between 5 and numel(tnrs)
    nobs  = size(data_lc,1);
    ntnrs = nan(nobs,1);
    for l = 1:nobs
        idxObs   = ~isnan(data_lc(l,2:end));                            % Data points on date l
        ntnrs(l) = sum(idxObs);
        if     l == 1
