...
% Motivation
Although it is well-known that U.S. monetary policy has effects beyond its borders, the extent to which it influences financial conditions in emerging markets it is not yet well understood.
...
The analysis of U.S. monetary policy spillovers has so far mainly focused on advanced economies, which partly reflects data constraints in emerging markets.
...
Section \ref{sec:analysisYCdcmp} analyzes the yield decompositions.
...
As explained in section \ref{sec:Estimation}, the parameters of the affine term structure models are estimated using end-of-month data.
Since the U.S. yield curve is the benchmark for the synthetic yield curves, these dates are the last business days of each month according to the U.S. calendar.

As explained in section \ref{sec:LPs}, the analysis on monetary policy spillovers requires daily data.
To correctly measure the responses of the emerging market yields to the U.S. monetary policy shocks it is key to have the timing right. 
In particular, changes in both nominal and synthetic yields need to correctly capture the Fed announcements.
Due to differences in time zones and in the timing of the announcements of U.S. monetary policy (14 hours New York time) and the Libor (11:45 hours London time), the forward premium for maturities equal or larger than one year is shifted one day forward for all countries.\footnote{ That is, the time subscript for the forward premium \(\rho\) in equation (\ref{eq:nLCsynt}) is \(\idxt + 1\) when \(\tnr \geq 1\).}
Similarly, the nominal yields of most countries are shifted forward by one day.\footnote{ The nominal yields Australia, Canada, Colombia, Japan, Malaysia, New Zealand are not shifted.}
...
This section describes the affine term structure model used to estimate the dynamics of the yield curve for each country in the sample. 
...
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
The nominal yields for each country can be decomposed once the affine model is estimated. 
...
The analysis focuses on the \(10\)-year maturity for the sake of brevity.
...
In sum, it does matter which curve is used (nominal or synthetic) to estimate the model in the case of EMs. 
