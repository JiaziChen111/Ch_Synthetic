...
This section shows the results of the decomposition of the nominal bond yields of emerging markets, and studies the dynamics of the components and their implications.
The results are compared against those of advanced economies. 
The analysis shows the benefits of using synthetic yield curves and survey data.

\iftoggle{fulldraft}{					% Turn it on/off in packages.tex

\subsection{Descriptive Results} \label{sec:results}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex

Assessment of the fit:
3 first PCs explain more than 99.5\% and 99.9\% of the variation in synthetic and nominal yields for EMs and AEs respectively. However, level plays a relatively bigger role in AEs, while slope and curvature play a relatively bigger role in EMs.
%	PC1				PC2			PC3			PC1-PC3
%	88.1115    9.8313    1.5731   99.5159
%	95.8896    3.7107    0.3016   99.9019
Plot of 10Y yObs vs yQ.
...
Cases HUF, MXN, MYR, PHP: w/o surveys implied expectations of rate (yP) were too low and so TP too high. W/ surveys expectations were higher and so TP lower.
	...
	TPsynt vs TPsvy
Similar to checking yP vs surveyCBP. TPsynt tracks TPsvy very closely.
...
USTP reacted similarly to TPs of AEs and EMs.
	...
	Is there a trend in yP?
There seems to be a downward trend in AEs but not in EMs.

	is EM real rates similar? Is there a trend? Compare yP, svycpi, realrt
For many countries it fluctuates around 0 and 1\%. BRL and MXN are the exception, their real rates fluctuate around 6 and 2.5\%.
...
Is regional (Asian and European) or global?
	...
	YC decomposition
For most of the countries yP is the largest component and is relatively stable. Reflecting IT regime?

	Correlations of components with alternative measures
TPsynt correlates positively with the residual from 10Y on 2Y and yP correlates positively with 2Y.

	TP correlations (reasonable TP estimate) vs BRP corrs
In general, TPsynt correlates negatively with LCCS and positively with INF, USTP and the VIX.
...
Correlation of EM TP w/ USTP is 62\% and AE TP w/ USTP is 77\%.
Correlation of EM TP w/ VIX is 32\% and AE TP w/ USTP is 37\%.

	commonalities (TPs, yP, LCCS) all, ST vs LT.
A global factor plays a role in the variation of both AEs and EMs to varying degrees.
Yields and YC components are not as tightly connected in EMs as they are in AEs.
Pooling all yields together, there is a global-level factor that explains around 50\% of the variation in EMs and 90\% in AEs. This is similar for the 1Y yields, whereas for the 10Y yields, a global factor explains around 95\% in AEs and 40\% in EMs. These percentages are largely the same after the GFC.
A global factor explains around 40\% of the variation of all 3 components of yield in EMs. For AEs, yP 90\% and TP 80\%. After the GFC, the variability explained by a global factor increased for the TP in AEs and for yP and TP in EMs.
The role of a global component for ST yields is larger than for the LT yields, but a global component is more relevant for the components of the LT yields than of the ST ones.

	How much the US factor explains TP? Is there a non-US common factor?
10Y USyP and USTP explain around 70\% of the variation in yP and TP in AEs; for EMs, they explain 30\% and 40\%, respectively. after the GFC, these numbers largely remained for the TP and decline for yP.

A non-US common factor explains b/w 50 and 85\% of the variation in yP and TP both ST and LT. These percentages fluctuate b/w 35 and 50\%. The importance of a non-US factor slightly increase after the GFC.

These results show that while the yields of EMs are globally connected, they are less so than the yields of AEs.
Several factors can explain these patterns. Some of them include segmented markets, capital controls, home bias, regional differences.
Local factors therefore seem to play a bigger role in the yields of EMs, and its components, than they do for AEs.

Kalemli-Ozcan argues that US spillovers work through changes in risk premia. In particular, expected rate in EMs react to changes in USTP. 
On average, USTP10 explains around 40\% of the variation in yP in both EMs and AEs.
For EMs, it explains more than USyP itself, unlike the case for AEs before GFC, but after GFC USTP10 explained more than USyP in both AEs and EMs.


\subsection{Emerging Markets Bond Risk Premia, Yield Curve Decompositions}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex


\subsection{Assessment of Term Premia Estimates}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
