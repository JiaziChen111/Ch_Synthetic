...
\subsection{Informational Deficiency} \label{sec:Identification}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
...
The connectedness index shows no upward trend for either the nominal or synthetic yields nor their components.\footnote{ \cite{ACDM:2019} document that the increase in the connectedness of the yields of advanced countries has been driven by an increase in the connectedness of their term premia.}
Moreover, the level of the index for the synthetic yields is higher than that for the nominal yields, suggesting that credit risk is indeed a more idiosyncratic component of the yields.
% (see figure \ref{subfig:dyindex10ydcmp}).
...
%To do this, 
I use the decomposition based on the \cite{KimWright:2005} model, which decomposes the yields into an expected future short rate and a term premium. The model addresses the small sample problem using survey forecasts of future interest rates.
...
\cite{Rey:2013} highlights the role of the Cboe's volatility index (Vix) as important driver of the global financial cycle.
...
\cite{BakerBloomDavis:2016} construct a news-based economic policy uncertainty (EPU) index that has been replicated for different countries. 
Unfortunately, it is only available for five of the emerging markets in the sample,\footnote{ Brazil, Colombia, Mexico, Russia and South Korea.} so its not included as an explanatory variable to account for domestic political uncertainty.
...
The exchange rate is standardized for each country over the sample period.
...
\subsubsection{Identification of Monetary Policy Shocks} \label{sec:USMPS}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
...
Unlike \cite{FerrariKearnsSchrimpf:2017} who include releases of minutes and speeches by Fed officials, the dataset here only includes monetary policy announcements.
...
The shocks are not correlated; notwithstanding, there were large easing path and LSAP shocks on March 18, 2009.
In general, the Fed is more aggressive in stimulating the U.S. economy as can be seen in the table, easing shocks are larger on average and more common than tightening shocks.


\subsubsection{Monetary Policy Spillovers} \label{sec:LPs}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
...
The regressions are done for the 2- and 10-year yields and each of their components (the expected short rate, the term premium and the credit risk compensation) for every type of monetary policy shock.\footnote{ By definition, the shocks are unanticipated by the market and thus there is no need to control for past or future shocks. Also, by construction, the shocks are not correlated among themselves so, there is no need to control for the other types of shocks.}
