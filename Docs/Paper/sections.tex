...
which in part reflects data constraints for emerging markets. 
Not so long ago, emerging markets 
%these countries 
used to issue bonds at short maturities and in foreign currency.
%So far, 
% now it is more common for them to issue bonds with longer maturities and in local currency.

% Research question
Research questions: How EM bond yields respond to US MP? Does US MP affect expectations of future policy rates in EMs? Did US UMP reduce TP abroad? Is there a risk premium channel from US to EMs? Do ST and LT yields respond differently?

Requirements to answer those questions: decompose EM yields, to adequately estimate TP: take into account credit risk and use surveys, methodology that enables event studies (estimation with daily frequency).
...
Since bond yields compensate investors for different risks, decomposing them provides 
...
These synthetic yield curves can be seen as being free of credit risk and 
...
%two common concerns when working with data from emerging markets.
...
The first contribution is to the large literature on term structure models by pioneering their application on synthetic yields, and to the literature that decomposes the bond yields of emerging markets \citep*{BlakeRuleRummel:2015,ACDM:2019} by correcting for credit risk and addressing the small sample problem. 
%The literature harness the synthetic yields in different contexts.
Synthetic yields have been used recently to study the deviations from covered interest parity (CIP).\footnote{ \cite*{DuTepperVerdelhan:2018} show that there are persistent and systematic deviations from CIP reflecting a higher regulatory burden for financial intermediaries. 
...
Rather than concentrating on the CIP deviations, this paper focuses on the synthetic yields themselves to decompose the nominal bond yields of emerging markets more finely.
...
This paper also contributes to the literature on the international comparison of sovereign yields \citep{DahlquistHasseltoft:2016}, which has so far mainly focused on advanced economies. 
...
Finally, the paper contributes to the literature on the international spillover effects of the monetary policy in the U.S. It extends the results in \citet*{GilchristYueZakrajsek:2019} by studying the effects of U.S. monetary policy shocks not only on sovereign yields but on its components, as in \cite*{CurcuruKaminLiRodriguez:2018} and \cite{ACDM:2019} but for the components of the yields of emerging markets.\footnote{ \cite*{HofmannShimShin:2019} study the link between the U.S. monetary policy, the exchange rate and the credit risk premium in emerging markets.}
...
Section \ref{sec:LCYCs} explains how to construct the local currency yield curves. Section \ref{sec:Methodology} presents the affine term structure model. Section \ref{sec:YC_dcmp} reports the yield decompositions and the analysis of the components. 
Section \ref{sec:USspillovers} studies the comovement of the sovereign yields of emerging markets.
%Section \ref{sec:spillovers} studies the U.S. monetary policy spillovers to the yields of emerging markets. 
...
This section explains how to construct the local currency (LC) yield curves of emerging markets, both nominal and synthetic.
...
For maturities equal or larger than one year, the forward premium is calculated using cross-currency swaps (XCS) since outright forwards are less liquid.\footnote{ Although credit default swaps (CDS)---financial derivatives aimed to protect investors against default by a bond issuer---capture credit risk in the medium to long term \citep{PalladiniPortes:2011}, they are not used in this paper for several reasons. First, credit risk is not eliminated, it simply shifts from the bond issuer to the CDS seller, i.e. there is counterparty credit risk. Second, a credit event is not clearly defined; for instance, borrowers can intentionally circumvent the CDS payout. Third, investors do not need to hold the underlying bond to buy a CDS, so there is a chance for market manipulation.}
...
Notice that the construction of the synthetic yield curve \(\yLCsynt\) does not require to know the nominal yield curve \(\yLCnom\),\footnote{ For the U.S., \(\yUSsynt = \yUS\) since there is no forward premium for the USD relative to the USD.}
...
The dataset comprises nominal and synthetic yields.
The data is available daily but the baseline results are obtained using end-of-month data from January 2000 to January 2019.\footnote{ Since the U.S. yield curve is the benchmark for the synthetic yield curves, these dates are the last business days of each month according to the U.S. calendar.}
%; nevertheless, the analysis on the international spillovers of the U.S. monetary policy uses daily data.
...
This small sample problem is addressed with surveys of professional forecasters in section \ref{sec:Estimation}. 
...
such that the corresponding continuously compounded yield of the bond is an affine function of those factors:
...
\cite{Guimaraes:2014} compares the three approaches and concludes that surveys provide robust decompositions of the U.S. and U.K. yield curves; for instance, the term premium estimated with the aid of surveys remains essentially the same after varying the number of pricing factors (from 3 to 5) and sample periods, unlike the other alternatives.

The importance of survey data is particularly relevant to get reliable decompositions for the bond yields of emerging markets since they help to overcome small sample sizes and regime changes.
...
As mentioned before, to estimate the affine model, the relevant yield curve for advanced countries is the nominal one \(\yLCnom\), whereas for emerging markets it is the synthetic one \(\yLCsynt\).
...
To strike a balance between these two considerations, I follow \cite{KimOrphanides:2012} in fixing \(\sigma_s\) at a conservative level of 75 basis points. This essentially means that survey data is seen as a noisy measure of expectations for the short rate.\footnote{ When \(\sigma_s\) is allowed to be estimated, its average value among all emerging markets is 24 basis points. For most of the countries in the sample, the estimated term premia remains largely the same.}

\subsubsection{Estimation with Daily Data}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
The parameters of the model are estimated with end-of-month data because there is noise at the daily frequency that can undermine the estimation. 
However, once the parameters have been estimated with monthly data, they can be used to estimate the model at the daily frequency \citep{ACM:2013}.

The procedure explained above gives estimates for the parameters and the pricing factors.
Principal component analysis on bond yields generates a matrix of weights or loadings for the different maturities to construct each principal component.
...
Finally, the estimated parameters from monthly data along with the daily pricing factors are used to fit and decompose the yields at the daily frequency.
...
\section{Yield Curve Decompositions} \label{sec:YC_dcmp}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
\iftoggle{cboxes}{	   				  % Turn it on/off in packages.tex
	\begin{boxeditems}
...
This section reports the decomposition of the nominal yields of emerging markets and the analysis of the components.
The results are assessed against those of advanced countries. 
The analysis highlights the benefits of using synthetic yields and survey data when studying the yields of emerging markets.
...
The survey-augmented model is estimated using the synthetic yields of emerging markets, and the yields-only model is estimated using the nominal yields of advanced countries.\footnote{ For Israel and South Africa, for which no survey data is available, the yields-only model is estimated using their synthetic yield curves.} 

In all cases, the pricing factors are the first three principal components (PCs) of the respective yield curves, which are commonly known as level, slope and curvature \citep{LittermanScheinkman:1991} .
On average, the first three PCs explain more than 99.5 and 99.9\% of the variation in the synthetic yields of EMs and the nominal yields of AEs, respectively.
Interestingly, the level factor plays a relatively bigger role in AEs than in EMs (96\% vs 88\%), as a consequence the slope (4 vs 10\%) and curvature (0 vs 2\%) factors play a relatively bigger role in EMs.
...
Figure \ref{fig:s_ylds_ssb_yQ} exemplifies the fit of the model for the 10-year synthetic yields of EMs. As it can be seen, fitting errors are small, indicating that the affine model successfully captures the dynamics of the yields.
...
\subsection{Decomposing EM Yields}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
Once the affine model is estimated for each country, the nominal yield curves of EMs can be decomposed into three parts.
...
The most immediate one is that the credit risk and the term premia are both time-varying. In addition, for most countries, the term premium plays a relatively bigger role than the credit risk premium in the dynamics of the EM yields.
A more in-depth discussion of these decompositions follows below after assessing the sensibility of each component.
...
The premium is thus a valid measure of credit risk that is far from perfect, but definitely better than ignoring credit risk. 
...
\subsection{Analyzing the Components}
...
One of the goals of those policies was to lower long-term bond yields and, in particular, the term premium.\footnote{ \cite{Kuttner:2018} discusses the effects of the QE announcements on U.S. bond yields.}
...
It is noteworthy that more than half of the EMs in the sample experienced negative term premia---which implies that investors considered EM bond yields as hedges and were therefore willing to give up some return in their EM bond investments.
...
The expected long-term real interest rate can be obtained as the difference between the long-term expectation for the short rate implied by the model and the long-term inflation forecast from Consensus Economics.

Figure \ref{fig:realrt_LT} plots the expected real rates of the EMs. 
The evidence for AEs shows that real interest rates have converged across countries and that they have declined to zero over the past years \citep*{HolstonLaubachWilliams:2017}.
It is remarkable that for all but one of the EMs their real rates fluctuate around similar levels of 0 and 2\%.\footnote{ The real rate of Brazil fluctuates around 6\%.}
This evidence shows that, after correcting for credit risk and inflation, the real interest rates of EMs behave similarly to the real rates documented for AEs.
\cite{Obstfeld:2020} discusses the global determinants of real interest rates, including the increase in savings from East Asian countries following the regional crises of the late-1990s.

\subsubsection{Bond Risk Premia in EMs}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
Since the credit risk premium is statistically different from zero \citep{DuSchreger:2016JoF}, the nominal yield curve is not free of credit risk.
Therefore, the decompositions obtained by estimating the affine model using nominal yields will provide biased estimates of the expected short rate and the term premium.

This shows that while bond risk premia is usually associated with the term premium in AEs, for emerging markets they are two different concepts.
...
By estimating the model using synthetic yields and surveys, I am then able to estimate a genuine term premium.
...
In sum, it does matter which curve is used to estimate the term premia in EMs. 
In particular, there are gains by using synthetic yields to account for credit risk in EMs.
...
When EMs face difficulties servicing their debt, they can either default or inflate away their debt, which can be referred to as explicit and implicit default. % Galli  (2020) 
...
\section{Spillovers of U.S. Monetary Policy to Emerging Market Bond Yields} \label{sec:USspillovers}
%	commonalities (TPs, yP, LCCS) all, ST vs LT.

This section analyze the reaction of the sovereign yields of emerging markets to U.S. monetary policy shocks. 

\subsection{U.S. Monetary Policy Shocks}
HF identification of MPS.

Windows: 15 minutes before to 1 hour and 45 minutes after each monetary policy meeting since 1994.\footnote{ It is common to exclude from the analysis the meeting of September 2001 that followed the terrorist attacks earlier in the month. See \cite{GSS:2005a} and \cite{NakamuraSteinsson:2018JEP}.}

Target: The change in the current federal funds rate future is a measure of surprises in the policy rate target (MP1). This are referred to as the target shocks. This is in line with the shocks described by \cite{Kuttner:2001}.

Path: After 2008 the change target shocks became essentially zero. The change in the 8 quarters ahead eurodollar future contract (ED8) is a measure of surprises in the 2-year yield.\footnote{ The change in the 4 quarters ahead eurodollar future contract (ED4) could also be used to measure path shocks. However, after 2011 intraday changes in ED4 became essentially zero since market participants expected the policy rate to remain at zero for at least a year. That situation did not happen to ED8.} I regress the ED8 intraday change on MP1. The residuals from this regression are referred to as the path shocks. The target and path shocks are similar to the shocks described by \cite{GSS:2005a}.
Correlation of shocks?

LSAP. Following \cite{Swanson:2018}, I regress the intraday change in the 10-year yield on the target and path shocks the residual of this regression is referred to as the term premium of LSAP shock.
...
The effects of U.S. monetary policy shocks on the yields of emerging markets are estimated using panel local projections.
The yield curve decompositions can be used to further analyze the effects of such shocks on each of the components, namely the expected short rate, the term premium and the credit risk premium.

Following \cite{Jorda:2005}
...
Categories of results: spillover index and PCA, inflation uncertainty and tp link, panel LPs.
...
This paper proposes a novel decomposition of the sovereign yields of emerging markets into an expected future short-term interest rate, a credit risk premium and a (pure) term premium. This is achieved by using synthetic local currency yields to estimate affine term structure models augmented with survey data.

Several new insights about the dynamics of bond yields in emerging markets were discussed.
