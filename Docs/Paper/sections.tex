...
In essence, survey data is seen as a noisy measure of expectations for the short rate.\footnote{ When \(\sigma_s\) is allowed to be estimated, its average value among all emerging markets is 41 % 24 before SOE
basis points.%\footnote{ 24 bp excluding IDR.} 
...
This highlights the benefits of using synthetic curves and survey data when analyzing the yields of emerging markets.
...
As is common in the literature \citep{JSZ:2011}, the pricing factors for all countries are the first three principal components (PCs) of the respective yield curves, they are commonly known as level, slope and curvature \citep{LittermanScheinkman:1991}.
...
Interestingly, the level factor plays a relatively bigger role in advanced countries than in emerging markets (96\% vs 88\%), so that the slope (4 vs 10\%) and curvature (0 vs 2\%) factors are relatively more important in EMs.
...
In general, emerging market yield curves are less smooth than those of advanced countries, likely due to a shallower investor base.
...
Unless otherwise stated, the analysis focuses on the \(10\)-year maturity for the sake of brevity.
...
First, the main component of the nominal yields of most countries is the expectation of the future short rate.
Moreover, a downward trend over the sample period can be seen for most of the countries, consistent with evidence for advanced countries \citep{ACDM:2019}.
...
%Nevertheless, 
the relative importance of the two premia varies by country and can even change over time as can be seen, for instance, for Hungary and the Philippines.
...
While the (bond) risk premium is usually associated with the term premium in advanced countries, they are two different concepts for emerging markets.
...
An alternative measure is the residual from regressing the 10-year yield on the 3-month yield (both synthetic); on average, its correlation with the model-implied term premium is 72\%.

The literature for advanced countries documents a downward trend in their term premia, which owes in part to a decline in inflation uncertainty \citep{Wright:2011}.
\cite{ACDM:2019} find a similar decline for emerging markets but without taking into account credit risk.
Figure \ref{fig:ny_dcmp} shows that their term premia indeed decreased over the sample period even after correcting for credit risk, although not equally widespread since a few exceptions can be seen; for instance, the term premium for Malaysia, Turkey and South Africa did decline earlier in the sample but increased again afterwards.
...
It is noteworthy that the phenomenon of a negative term premia---when investors considered bonds as hedges and are therefore willing to give up some investment return---is not restricted to advanced countries.
Figure \ref{fig:ny_dcmp} shows that some countries in emerging Asia experienced episodes of negative term premia after the global financial crisis.
%---unlike the term premia in AEs.
Similarly, since mid-2011 the U.S. term premium based on the methodology of \cite{KimWright:2005} has been negative most of the time, fluctuating between \(-1\) and \(0\)\%, as is the case for other advanced countries, which is documented by \cite{Wright:2011}.
...
comparing the term premia across maturities per country
%, known as the term structure of term premia, 
indicates that they generally increase with maturity. 
Investors require a higher compensation for holding long-term bonds when they are seen as riskier than short-term bonds.
...
Figure \ref{fig:bsl_tp_ts} shows that whereas countries in emerging Asia experienced negative term premia on the long end of their yield curves, different countries experienced it on the short end.
International investors seem to have had a preference for short-term LC bonds after the global financial crisis.
...
The role of the credit risk premium in explaining yield variation is non-negligible in general.
On average, it is positive for all the countries. 
...
Therefore, it does matter which curve is used (nominal or synthetic) when decomposing the yields of emerging markets. 
...
\subsubsection{Risk Premia in Emerging Market Yields}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex

%The components of the bond risk premium capture different risks. Indeed, 
Given that the two premia help explain yield variation in emerging markets, a natural question is whether and how they are related.

The term premium correlates negatively with the credit risk premium for 11 out of the 15 emerging markets with an average of \(-0.38\).\footnote{ The correlation is not statistically significant for Colombia, Indonesia, Poland and Russia at the 10\% level.} 
When emerging markets face difficulties servicing their bond payments, they can either default or inflate away their debt, which can be referred to as explicit and implicit default. % Galli  (2020) 
...
This evidence points to a trade-off between explicit and implicit defaults for most emerging markets.
...
\subsection{Connectedness} \label{sec:globalPC}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
