...
% Motivation
Financial conditions across countries are widely believed to be interconnected. Yet, the extent to which the local financial conditions in emerging markets are interrelated has not been widely explored, in part due to data availability. Not so long ago, these countries used to issue bonds at short maturities and in foreign currency, and so the international comparison of sovereign yields has mainly focused on advanced economies.
% now it is more common for them to issue bonds with longer maturities and in local currency.

% Research question
This paper decomposes the local currency sovereign yields of emerging markets and compares their properties against advanced countries. This is relevant because
...
Bond yields compensate investors for different risks; decomposing them thus provides further insights about 
...
\cite{DuSchreger:2016WP} and \cite{Galli:2020} provide theoretical explanations for these episodes.
...
These synthetic yield curves can be seen as being free of credit risk. Like the nominal yields of advanced countries, the synthetic yields of emerging markets can thus be decomposed into a 
%Therefore, I use the synthetic rather than the actual curves of 15 emerging markets to estimate their 
future expected short-term interest rate and a term premium, which compensates investors for the uncertain return of locking their money over the life of the bond.
The difference between the nominal (or actual) yields and the synthetic ones captures the credit risk premium in emerging markets \citep{DuSchreger:2016JoF}.
...
\cite{Guimaraes:2014} shows that surveys on interest rate forecasts provide robust decompositions of the U.S. and U.K. yield curves.\footnote{ For instance, he finds that the term premia estimated with the aid of surveys remain essentially the same after varying the number of pricing factors (from 3 to 5) and the sample periods, even with a sample starting in 1972, which includes the U.S. Great Inflation period.} 
Surveys are especially relevant for emerging markets because they help to overcome both small sample sizes and regime changes, two common concerns when working with data from emerging markets.
...
The main component of the 10-year yields is the expected short rate, followed by the term premium, whose size more than doubles the credit risk premium. 
...
First, term premia declined globally following the quantitative easing (QE) announcements and, in general, increased with the taper tantrum; 
even the expectation part of the short rate in some countries coincided with the QE announcements.
Second, the expected short rate has been stable in all of the emerging markets, even declining in some cases; importantly, all but one of the countries in the sample have adopted (before or during the sample period) an inflation target regime.
Third, the decomposition also allows me to study the expected real interest rate in emerging markets, the difference between the long-term expected short rate and the inflation forecast.
These real rates fluctuate between 0 and 1\%; moreover, in line with the evidence for advanced countries, there is a declining trend in the real rates of some countries.
Finally, a global factor plays a role in the variation of yields in both advanced and emerging countries, but the bond yields in emerging markets are not as tightly connected as they are in advanced countries;
further, the role of the global factor is different throughout the yield curve, its importance is larger for short-term yields than for the long-term yields, but it is more relevant for the components of the longer-term yields.
...
This paper contributes to the large literature on term structure models by pioneering their application on synthetic yields, and to the literature that decomposes the bond yields of emerging markets \citep*{BlakeRuleRummel:2015,ACDM:2019} by correcting for credit risk and addressing the small sample problem. 
...
The rest of the paper proceeds as follows. Section \ref{sec:LCYCs} explains how to construct the local currency yield curves. Section \ref{sec:Methodology} describes the affine term structure model. Section \ref{sec:comovement} reports the yield decompositions and their analysis. 
%Section \ref{sec:spillovers} studies U.S. monetary policy spillovers to the bond yields of emerging markets. 
...
Surveys have good forecasting properties. However, it is important to acknowledge that they are not a panacea. For instance, surveys might not represent the market expectations or the expectations of the marginal investor.
...
Figures \ref{fig:wnCPI} and \ref{fig:wnGDP} respectively plot the long-horizon forecasts for inflation and real GDP growth from Consensus Economics.
Only Brazil and Turkey show an upward trend in their inflation expectations; for the rest of EMs, they have been stable or even declining.
Meanwhile, for half of the EMs in the sample, there is a declining trend in their expected GDP growth. 
...
An alternative way to use the surveys from Consensus Economics is to assume that the emerging markets in the sample are small open economies, and so their real interest rate would be determined abroad. The forecast for the local short rate would then be equal to the forecast for the international short-term real interest rate plus the forecast for inflation at each of the available maturities. 
This approach requires to define the international real short rate and to have long-term forecasts for it.
As before, the U.S. can be the benchmark for all emerging markets. Long-term forecasts for it can be obtained as the difference between the respective forecasts for the short-term nominal interest rate and inflation.
This approach will be included in a future version of this paper.
...
This section shows the results of the decomposition of the nominal bond yields of emerging markets, and studies the dynamics and implications of the components.
The results are assessed against those of advanced economies. 
The analysis shows the benefits of using synthetic yield curves and survey data when analyzing the bond yields of emerging markets.
...
The affine term structure model is estimated separately for each country.
The survey-augmented model is estimated using the synthetic yield curves of emerging markets, and the yields-only model is estimated using the nominal yield curves of advanced countries.\footnote{ For Israel and South Africa, for which no survey data is available, the yields-only model is estimated using their synthetic yield curves.} 
Figure \ref{fig:s_ylds_ssb_yQ} exemplifies the fit of the model for the 10-year synthetic yields of EMs. Fitting errors are small.

In all cases the pricing factors are the first three principal components (PCs) of the respective yield curves.
\cite{LittermanScheinkman:1991} refer to these factors as level, slope and curvature.
On average, the first three PCs explain more than 99.5\% and 99.9\% of the variation in the synthetic yield curves of EMs and the nominal yield curves of AEs, respectively.
Interestingly, the level factor plays a relatively bigger role in AEs than in EMs (96\% vs 88\%), therefore the slope (10\% vs 4\%) and curvature (2\% vs 0\%) factors play a relatively bigger role in EMs.
%	\input{../Figures/Estimation/s_ylds_ssb_yQ}
...
\subsection{Yield Curve Decompositions}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
Once the affine term structure model is estimated for each country, the nominal yield curves of EMs can be decomposed into three parts.
The difference between the nominal and synthetic yield curves is a measure of the credit risk premium in EMs \citep{DuSchreger:2016JoF}.
The other two components come from the decomposition of the synthetic yield curves into an expectation of the future short-term interest rate and a term premium; this two-part decomposition is equivalent to how the nominal yields of AEs are decomposed.
The yields can be decomposed in this way at each maturity but, unless otherwise stated, the analysis focuses on the \(10\)-year maturity for the sake of brevity.
...
The most immediate one is that the credit risk and the term premia are both time-varying. In addition, for most countries, the term premium plays a relatively bigger role than the credit risk premium in the dynamics of EM sovereign bond yields.
A more in-depth discussion of these decompositions follows below after assessing each of the components.

%\subsubsection{Assessing the Components}

\paragraph{Expected Future Short Rate.} The model-implied expectation of the short rate for the 10-year maturity is assessed against the interest rate forecasts calculated as explained in section \ref{sec:SurveyData}. 
The model acknowledges that the survey forecasts are estimated by treating them as noisy measures of the expected short rate, it therefore does not rely too much on them.
Nevertheless, the model-implied expectations track the interest rate forecasts reasonably well, as can be seen in figure \ref{fig:svycbp_ssb_yP}.
%Figure \ref{fig:svycbp_ssb_yP} displays both measures.
%	\input{../Figures/Estimation/svycbp_ssb_yP}
Surveys do in fact help the model pinning down the parameters under the \(\Pmeasure\) measure.
When no survey data is used in the estimation, the model-implied expectations are weakly connected to the forecasts.
For some countries, the yields-only model-implied expectations of the short rate were too low and, thus, the estimated term premium was too high. 
Once survey data was incorporated into the estimation, the model-implied expectations increased in line with the interest rate forecasts and the estimated term premium decreased; it even became negative for some countries.
...
These results show the relevance of incorporating surveys in the estimation of the model for EMs, they help in dealing with the small sample problem characteristic in EM data.

\paragraph{Term Premium.} There are two model-free measures commonly used to assess the model-implied term premium.
...
Since the model-implied expectations track the interest rate forecasts very closely, as mentioned above, the model-implied and the survey-based term premia are very highly correlated.
...
\paragraph{Credit Risk Premium.} The dynamics of the credit risk premium are in line with the results reported by \cite{DuSchreger:2016JoF} who, in particular, show that it is highly correlated with the CDS of the respective country.
...
\subsection{Analyzing the Decompositions}
This section exploits the decomposition of the nominal yields in EMs to better understand their dynamics.

\subsubsection{Effects of U.S. Unconventional Monetary Policy}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
% Term Premia Responses to 
The decomposition of the nominal yields can be used to see the effects of the large-scale asset purchases that were part of the unconventional monetary policy (UMP) tools implemented by the U.S. Fed as part of its response to the global financial crisis (GFC).

One of the goals of those policies was to lower long-term bond yields and, in particular, the term premia.\footnote{ \cite{Kuttner:2018} discusses the effects of the QE announcements on U.S. bond yields.}
...
Figures \ref{fig:ssb_yP_QE} and \ref{fig:ny_yP_QE_AE}  show that, in general, expectations of future short rates declined during the QE announcements and increased with the TT announcement.
...
It is noteworthy that more than half of the EMs in the sample experienced negative term premia---which implies that investors considered EM bond yields as hedges and were therefore willing to give up some return in their EM bond investments---unlike the term premia in AEs.
For comparison, the U.S. term premium---estimated by 
\cite{KimWright:2005}---has been negative for most of the time since mid-2011, fluctuating between $-1$ and $0$\%.\footnote{ \cite{Wright:2011} reports the 5-to-10 forward term premium for all the AEs considered here, except Denmark; for six of them, their term premia turned negative even before the GFC. In the estimation, he augments the affine model with data from macroeconomic variables. This might support the case of supplementing the estimation of the affine model of AEs either with macro or survey data.} 
Moreover, this phenomenon is observed during and after the GFC, in some cases it coincided with the QE announcements.
Interestingly, for Brazil and Turkey the decline in their term premia between 2008 and 2013 happened even when their inflation expectations were increasing.
...
\subsubsection{Trends in Yield Components}
%	YC decomposition
The main component of the nominal yield curves of most EMs is the expectation of the future short rate (see figure \ref{fig:ny_dcmp}).
In fact, for some countries the expectation of the future short rate is relatively stable, whereas for others it even declined over the sample period.
...
Nevertheless, in addition to the declining trend in the term premia of both AEs and EMs since the start of the QE program, figure \ref{fig:ssb_tp_QE} also shows that for some EMs,\footnote{HUF, IDR, KRW, MXN, PHP, PLN} a declining trend in their term premia can be perceived even before the GFC; this is consistent with the evidence for AEs documented by \cite{Wright:2011} with data going back to 1990.\footnote{ He argues that this downward trend reflects a reduction in inflation uncertainty.}
The largest declines can be seen in Asian and European countries.\footnote{ HUF, IDR, KRW, PHP, PLN, THB.}
Interestingly, Mexico, Russia and Turkey experienced reversals in their term premia since they increased after declining to historic lows.
...
Finally, in addition to comparing the term premia across countries, one can also compare them across maturities per country, known as the term structure of term premia. In general, the term premium increases with maturity. As one would expect, when long-term bonds are seen as riskier than short-term bonds, investors would require a higher compensation for holding long-term bonds.

\subsubsection{Effects of Local Conditions}
The estimated term premia also seems to be related to local conditions.
Figure \ref{fig:ssb_tp_local} plots the term premium of some EMs along with the dates of relevant local events.\footnote{ For Brazil, the events highlighted are the announcements of capital controls in response to UMP announcements by central banks in AEs. For Colombia, the events are announcements of capital controls before the GFC. For Hungary, the events are the approval of the treaty to join the European Union, the adoption of an an explicit medium-term inflation target by the central bank, a demand by the European parliament to prevent the Hungarian authorities from breaching the EU’s founding values. For Indonesia, the event is the adoption of inflation targeting. For South Korea, the event is the announcement of measures to limit financial firms’ positions in foreign exchange derivatives to control currency flows. For Poland, the event is the approval of the treaty to join the European Union and the publication of a law threatening the independence of the judiciary. The events for Turkey are the adoption of inflation targeting by the central bank and the killing of the journalist Jamal Khashoggi who disappeared after he visited the consulate of Saudi Arabia in Istanbul.}

\subsubsection{Is There a Global Neutral Real Interest Rate?}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
The neutral real interest rate is an important concept in monetary economics.
It is a key reference for inflation-targeting central banks.
However, it is an unobserved variable.
Estimates of it generally focus on AEs.
The evidence shows that real interest rates have converged and declined over the past decades.

The decomposition of the nominal yields of EMs, however, opens the door to glimpse the long-term real interest rates faced by the EMs in the sample.
The long-term real interest rate can be obtained as the difference between the long-term expectation for short rate implied by the model and the long-term inflation forecast reported from Consensus Economics.

Figure \ref{fig:realrt_LT} shows that the expected real rate in EMs is time-varying. There also seems to be a declining trend for some of the countries in the sample.
Moreover, the real rate for many EMs fluctuates around 0 and 1\%. 
Notwithstanding, the expected real rates for Brazil and Mexico fluctuate around 6 and 2.5\%, respectively.
This evidence is line with what has been documented for AEs.

\subsubsection{Bond Risk Premia in EMs}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
The literature that estimates affine term structure models for EMs is scant relative to the literature for AEs.
Furthermore, the applications to EMs use the nominal yields as the input for the affine model.
This implicitly assumes that the nominal yield curve is free of credit risk, which \cite{DuSchreger:2016JoF} show is not the case.
Therefore, the decompositions obtained by estimating the model to the nominal yields provides biased estimates of the expected short rate and the term premium.

In EMs, bond risk premia and term premia are not the same.\footnote{ For AEs, they are sometimes used interchangeably.}
Supplementing with survey data the model estimated with nominal yields produces an expected short rate similar to the one obtained with synthetic yields.
% correlation?
Therefore, the difference between nominal yields and the expected short rate obtained with the survey-augmented model is actually a measure of bond risk premia in EMs.
By estimating the model using synthetic yields and surveys, I am able to generate a genuine term premium for EMs.
As a result, I define the bond risk premia in EMs as the sum of a term premium and a credit risk premium.\footnote{ This measure of the bond risk premia is highly correlated with the residual obtained by estimating the model with nominal yields.}

The two components of the bond risk premium in EMs play an important role in its dynamics.
Figure \ref{fig:brp_dcmp} shows the decomposition of the bond risk premia.
The role of its two components varies by country.
For some countries the relative importance of both components changed over time; for instance, for Hungary and South Korea, the term premium declined relative to the credit risk premium.
...
In sum, it does matter which curve is used to estimate the term premia in EMs. There are gains by using synthetic yield curves to account for credit risk in EMs.
...
\subsubsection{Term Premia and Measures of Risk and Uncertainty }
Term premia captures the uncertainty risk of investing in bond yields.
It is expected then for the term premium to be related to risk and uncertainty measures.

The components of the bond risk premium capture different risks.
Indeed, for all EMs their estimated term premium correlates negatively with their credit risk premium.\footnote{ The correlation is not statistically significant only in the case of Indonesia.} 
...
%TPsynt of COP and MXN correlates positively with EPU. For KRW it comove negatively, its LCCS is what comoves positively with the EPU.
...
\subsection{Comovement of Sovereign Yields}
%	commonalities (TPs, yP, LCCS) all, ST vs LT.
...
Pooling all yields together, there is a global-level factor---associated with the first PC---that explains around 50\% of the variation in EMs but 90\% in AEs. 
...
A similar conclusion can be drawn for the yield components.
...
\cite{Kalemli-Ozcan:2019} further argues that U.S. spillovers work through changes in risk premia, especially for EMs. % at the short end 
...
Future versions of the paper will explore the effects of U.S. monetary policy shocks, identified with high frequency data, on the components of the yields of emerging markets.

The role of debt capital flows in explaining the negative term premia estimated for some emerging markets will also be explored.
...
\begin{landscape}
	...
	\input{../Figures/Estimation/s_ylds_ssb_yQ}	% \ref{fig:s_ylds_ssb_yQ}
	\input{../Figures/Estimation/ny_dcmp}
	\input{../Figures/Estimation/svycbp_ssb_yP}
	\input{../Figures/Estimation/ssb_tp_QE}
	\input{../Figures/Estimation/ny_tp_QE_AE}
	...
	\input{../Figures/Estimation/realrt_LT}
