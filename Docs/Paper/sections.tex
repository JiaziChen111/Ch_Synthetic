...
% Motivation
Financial conditions across countries are widely believed to be interconnected. Yet, the extent to which the local financial conditions in emerging markets are interrelated has barely been explored, in part due to data availability. Around the turn of the millennium, these countries used to issue bonds at short maturities and in foreign currency.
...
\iftoggle{cboxes}{	   				  % Turn it on/off in packages.tex
	\begin{boxeditems}
		\item Order of model: no arbitrage, SDF exists, Q exists and is different than P, price function in terms of expectation under Q, dynamics of the factors under Q and short rate dynamics, price function under Q, final component: for term premium SDF depends on factors (Duffee), SDF dynamics, factor dynamics under P.
		\item Sources: Brandt and Chapman, Guimaraes, Lloyd.
	\end{boxeditems}}{}
...
The no-arbitrage assumption implies the existence of a strictly positive stochastic discount factor (SDF), \(\SDF\), that prices all assets. Accordingly, the bond price today equals the expectation of tomorrow's discounted price, it is thus recursively defined as follows:\footnote{ Since the price at maturity of a zero-coupon bond is $1$, recursive substitution of equation (\ref{eq:nPzero}) implies that today's price equals the expectation of the product of SDFs over the life of the bond, $\Pzero = \SDFprod$.}
	\input{../Equations/nPzero}
\noindent where \(\ExpP [\cdot]\) denotes the expectation of a random variable taken using the actual or physical probability measure, \(\Pmeasure\).

The existence of the SDF also implies that there exists a theoretical risk-neutral or risk-adjusted pricing measure, \(\Qmeasure\), that is different from the \(\Pmeasure\) measure that generates the data.

A discrete-time affine term structure model assumes that the dynamics of a \(\Xdim \times 1\) vector unobserved pricing factors or state variables \(\Xvars\) follow a first-order vector autoregression, VAR(1), under the risk-neutral measure \(\Qmeasure\):
	\input{../Equations/nXvarsQ}
\noindent where \(\XmuQ\) is a \(\Xdim \times 1\) vector and \(\XPhiQ\) is a \(\Xdim \times \Xdim\) matrix of parameters, $\XSigma$ is a \(\Xdim \times \Xdim\) lower triangular matrix with positive diagonal elements, and \(\error\) is a \(\Xdim \times 1\) independent and identically distributed, zero-mean normal vector with covariance equal to the identity matrix conditional on the state vector; this is written as \(\error | \Xvars \overset{\text{iid}}{\sim} \Normal_{\Xdim} \left(0,I\right)\). 

of the vector of state variables $\Xvars$ are assumed to evolve under the physical measure $(\Pmeasure)$ according to the following :
\input{../Equations/nXvarsP}

drives the dynamics of the one-period interest rate $\rShort$, the $\Xdim \times 1$ market price of risk $\riskprice$ and the logarithm of the SDF in an affine form as follows:
...
are governed by the following parameters:
	\input{../Equations/uXmuStar}
	\input{../Equations/uXPhiStar}
The log price and the continuously compounded yield of a risk-free zero-coupon bond in this model are then affine functions of the state variables $\Xvars$:
	...
	\input{../Equations/uYaffine}
\noindent where the scalar \input{../Equations/functionAn} and the $1 \times \Xdim$ vector \input{../Equations/functionBn}. These coefficients can be computed recursively as follows after combining the no-arbitrage condition and the functional form for bond prices:
	\input{../Equations/uAffineA}
	\input{../Equations/uAffineB}
...
Second, I assume that the estimated parameters in equation (\ref{eq:nTaylorRule}) apply to each of the survey maturities \(\tnr = 1, 2 \ldots, 5, 10\) for inflation and real GDP growth, and thus the expected future policy rate is calculated as:\footnote{ Under stationarity, $\mathrm{E}(\rShort) = \mathrm{E}(\rShortlag)$. The long-term forecasts are allocated to the 10-year maturity.}
	\input{../Equations/nYsurvey}

The model is augmented with these survey expectations on the last day of the month for which the data was published.\footnote{ From 2001 to 2014, data for countries covered in the Eastern European release is available in March and September; starting in October 2014, it is released on April and October. For the rest of emerging markets the forecasts have always been released on April and October.}

Finally, the model-free estimate for the $\tnr$-period term premium is obtained as the difference between \(\yLCnom\) and \(\ySvy\).
...
\section{Yield Curve Decompositions} \label{sec:comovement}
\iftoggle{toclinks}{\gototoc}{} % Turn it on/off in packages.tex, command in macros.tex
\iftoggle{cboxes}{	   				  % Turn it on/off in packages.tex
	\begin{boxeditems}
