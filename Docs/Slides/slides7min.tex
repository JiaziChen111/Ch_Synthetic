...
\note{Hi, my name is Pavel Solís. My JMP addresses the question: How does US MP transmit to the sovereign bond yields of EM?
	To answer the question, I first propose a decomposition of the yields that explicitly takes into account the risk that EM might default. I then use the decomposition to understand the effects of US MP surprises on each component.}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \alert{Local} currency: \alert{90\%} of emerging market (EM) sovereign debt in 2018 %%\note{Table 1 IMF-WB staff note to G20 11/12.2 = 90\%.}
			\newline
		\end{itemize}
	\end{itemize}
...
\end{frame}
\note{It is well documented that US MP has worldwide effects. It influences, for instance, different asset prices like stocks, exchange rates and yields of bonds issued in F and LCs. However, the transmission of US MP to the LC bond yields of EM is not yet well understood, which is a relevant point given that 90\% of their debt is issued in LC.}
\note{My JMP studies those transmission channels.}
...
\end{center}
\end{frame}
\note{To understand the transmission channels, past work uses traditional decompositions, which focus on average expected future short rates and term premia.}
\note{In the figure, you can see a stylized decomposition of bond yields, which assumes that the bonds are free of default risk.}
\note{That is a reasonable assumption for bonds issued by AE but not for bonds issued by EM.}
\note{Indeed, a shortcoming of past work is to use this two-part decomposition for the yields of EM because it ignores a third channel: a compensation for the fact that EMs do default.}
...
\begin{frame}[label=SovereignDefaults]
	...
	\end{textblock*}
\end{frame}
\note{A widely held belief is that sovereigns rarely default on LC debt b/c they can `print' their own currency.}
\note{The figure shows the number of sovereigns that have defaulted on FC and LC debt since 1960.}
\note{The number of defaults in FC are in blue. The number of defaults in LC are in red.}
\note{As you can see, defaults on LC debt do occur and are more common than is usually thought.}
\note{Just to give you a statistic, there have been more than 60 default episodes under domestic law since 1980.} % , bonds denominated in LC
\note{A famous example is Russia in 1998 but there are more recent examples like Barbados in 2018.}
...
\end{center}
\end{frame}
\note{In the paper, I systematically examine credit risk unlike previous work studying the YCs of EM.}
\note{I propose a three-part decomposition for their sovereign bond yields.}
\note{In addition to the two usual components, there is a third component that compensates for credit risk.}
\note{The figure shows a schematic three-part yield decomposition.}
\note{To explicitly account for credit risk, I use synthetic yields, which essentially swap the US YC into the LC of an EM. These synthetic yields can be considered free of credit risk, so traditional decompositions can be applied to them. The spread between the observed and the synthetic yields is the credit risk compensation.}


\begin{frame}
\frametitle{EM Yield Curve Decomposition}
\begin{center}
	\includegraphics[trim={0cm 0cm 0cm 0cm},clip,width=0.85\textwidth,height=0.9\textheight]{../Figures/Estimation/MX_dcmp.eps}
\end{center}
\end{frame}
\note{I decompose the yields of 15 EM using a standard ATSM augmented with survey data to obtain robust decompositions.}
\note{In the figure you can see how the decomposition looks like over time for the 10Y yield of Mexico, one the countries in the sample.}
\note{The solid blue line is... The dash-dotted orange line is... And the dashed yellow line is...}
\note{For all the countries in the sample, the decomposition captures local conditions. In the case of MX, for instance, the central bank started a tightening cycle following the 2016 US presidential election, in response to a depreciation of the currency. That move is reflected in an increase in ESR.}
...
\end{center}
\end{frame}
\note{To quantify the effects of US MP on each of the yield components, I follow the literature that identifies MP surprises using intraday data on asset prices around FOMC meetings. In the analysis, I distinguish between target, forward guidance and LSAP surprises.}
...
%		\item 
		...
		\begin{itemize}
			\item Response in EM lasts longer than in U.S.
			\newline
		\end{itemize}
...
%		\item 
		3. Unconventional policies \alert{limit} EM monetary autonomy along yield curve % <3>
		\begin{itemize}
			\item Global financial cycle more relevant at the long end
		\end{itemize}
%	\end{enumerate}
\end{frame}
\note{Three main findings summarize the transmission channels of US MP surprises to EM yields.}
\note{First, the response of EM yields is economically significant and amplifies over the month following a MP surprise, which can be attributed to a portfolio rebalancing channel and slow-moving capital.}
\note{Plus, the effects on EM yields last longer than on US yields.}

\note{Second, surprises in US MP lead to a reassessment of policy rate expectations and a repricing of interest and credit risks in EM.}
\note{In particular, I find that investors expect CBs in EM to follow the monetary stance of the Federal Reserve rather than counteract it. So if the Fed loosens or tightens financial conditions, EM CBs are expected to follow suit.}
...
\note{Finally, since the 2007-2009 crisis the GFCy -global comovement of risky asset prices- is stronger at the long end of the YCs of EM, which limits the influence their CBs can exert over their domestic YCs.}

\note{These results give rise to further research questions. For example, what tools do CBs in EM have at their disposal to respond to MP spillovers from abroad. I address questions of this nature in the other chapters of my dissertation.}
\note{Thank you for watching.}



%-LT yields comove more than ST ones-
