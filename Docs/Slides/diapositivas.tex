...
\title{Prima por Plazo y Riesgo de Crédito en Mercados Emergentes: El Rol de la Política Monetaria de EUA}
\date{Septiembre 2021}
\author{Pavel Solís}
\institute{Banco de México}

\note{Hello everyone, thank you to the organizers for including the paper.} %Please feel free to ask questions.
\note{In the paper, I study the transmission channels of US MP to the sovereign bond yields of EMs.}
...
%{\usebackgroundtemplate{\includegraphics[width=\paperwidth]{banxico_portada.png}}
%\maketitle	}
\maketitle

%\usebackgroundtemplate{\includegraphics[width=\paperwidth]{banxico_lamina.png}}
{\setbeamertemplate{frame footer}{*IMF-WB (2020). Recent Developments on Local Currency Bond Markets in Emerging Economies.}
\begin{frame}
	\frametitle{Política Monetaria de EUA Tiene Efectos en el Exterior}
	La política monetaria de EUA afecta los precios de activos fuera EUA
	\begin{itemize}
		\item Acciones \item Tipo de cambio
		\item \alert{Bonos}
		\begin{itemize}
			\item Moneda extranjera
			\item Moneda \alert{local}: Más de \alert{80\%} de deuda soberana de emergentes* % in 2018 %(IMF-WB)
			\newline
		\end{itemize}
	\end{itemize}
	...
	\vspace{-0.3cm}
	Entender canales de transmisión para \alert{enfrentar} efectos no deseados
	\begin{itemize}
		\item Descomposición tradicional de tasas supone riesgo de crédito nulo
	\end{itemize}
\end{frame}		}
...
\note{The reason is that government bond yields are key to local financial conditions b/c they are a benchmark against which other domestic assets, like corporate bonds and loans, are priced.}
...
\begin{frame}[label=SovereignDefaults]
	...
	\begin{textblock*}{5cm}(0.91\textwidth,0.95\textheight)
		\hyperlink{CreditRating}{\beamergotobutton{Calificaciones Crediticias}}
	\end{textblock*}
\end{frame}
...
\begin{frame}
	\frametitle{Este Artículo}
	¿Cómo \alert{descomponer} las tasas soberanas de mercados emergentes?
	\begin{itemize}
		\item Tomando en cuenta el riesgo de cédito
		\newline
	\end{itemize}
	
	¿Cómo se \alert{transmite} la política monetaria de EUA a tasas soberanas de emergntes?
		\begin{itemize}
			\item ¿Expectativas para la tasa de política en el futuro? 
			\item ¿Prima por plazo?
			\item ¿Riesgo de impago?
		\end{itemize}
\end{frame}
\note{This paper proposes a decomposition of the sovereign yields of EMs that *explicitly* takes into account the risk that EMs might default on their LC debt.}
\note{I then use the decomposition to address the main research question in the paper, which is: How does US MP transmit to the sovereign bond yields of EMs? So that I can empirically quantify the effects of US MP surprises on each component.}

\note{Three main findings summarize the transmission channels of US MP to EM yields. But since they involve the components of the yields, let me show you a couple of figures to fix ideas.}


\begin{frame}
\frametitle{Decomposición Traditional de la Curva de Tasas}
\begin{center}
	\includegraphics[trim={0cm 0cm 0cm 0cm},clip,width=0.85\textwidth,height=0.95\textheight]{../Figures/YC/ycdcmp_AEesp}
\end{center}
\end{frame}
\note{This figure shows a stylized two-part decomposition of bond yields. The YC shows the relationship b/w bond yields at different maturities at a point in time. By assuming that the bonds are free of default risk, the yields can be decomposed into two parts: an average expected future short rate and a term premium.}
\note{The term premium compensates investors for bearing interest rate risk.}
...
\begin{frame}
\frametitle{Decomposición Propuesta para la Curva Emergentes}
\begin{center}
	\includegraphics[trim={0cm 0cm 0cm 0cm},clip,width=0.85\textwidth,height=0.95\textheight]{../Figures/YC/ycdcmp_EMesp}
\end{center}
\end{frame}
...
\begin{frame}
	\frametitle{Efectos de la Política Monetaria de EUA en Tasas de Emergentes}
%	\begin{enumerate}
%		\item 
		1. Tasas de emergentes responden \alert{significativamente}, aunque con \alert{rezago} %over days
		\begin{itemize}
			\item Efecto puede durar más en tasas de emergentes que en tasas de EUA
			\newline
		\end{itemize}
		
		
%		\item 
		2. Los \alert{tres} componentes reaccionan a la política monetaria de EUA % shocks <2->
		\begin{itemize}
			\item Se espera una posutra monetaria similar en emergentes que en EUA 
			\item Prima por plazo en emergentes reacciona similar a la de EUA
			\item Implicaciones en para emergentes de la política monetaria de EUA
			\newline
		\end{itemize} 
	
%		\item 
		3. Políticas no convencionales \alert{limitan} influencia en curva de tasas % yield <3>
		\begin{itemize}
			\item Ciclo financiero global más relevante en la parte larga de la curva 
		\end{itemize}
%	\end{enumerate}
\end{frame}
...
\section{Curvas de Tasas}
\note{Is there any question? Let me start by explaining how to construct the YCs that will be decomposed later.}

\begin{frame}
\frametitle{Curvas de Tasas Nominales}

	Curvas de tasas par (Bloomberg) \(\rightarrow\) Curvas de tasas cupón cero (\(\yLCnom\))
	\begin{itemize}
		\item Pero \alert{riesgo de crédito} en \(\yLCnom\) %LC nominal yields of EM
	\end{itemize}
	
	\textbf{Estrategia}: Tasas \alert{sintéticas} en moneda local (\(\yLCsynt\)) \textit{sin riesgo de crédito}
	\begin{itemize}
		\item Converitr tasas en dólares a tasas en moneda local usando derivados 
	\end{itemize}
	
	\textbf{Supuesto}: Mercados financieros sin fricciones \citep{DuSchreger:2016JoF}
	\begin{itemize}
		\item Inversionistas tienen acceso a bonos de EUA y en moneda local
		\item Derivados no tienen riesgo de contraparte
		\item Tasas de EUA son libres de riesgo de incumplimiento
	\end{itemize}
...
\note{There are circumstances in which these assumptions migh not hold exactly. However, \cite{DuSchreger:2016JoF} show that these assumptions are a \alert{useful benchmark} to study credit risk in EM yields.} 
...
\frametitle{Curvas de Tasas Sintéticas}
%\vspace{-1cm}
\input {../Equations/uLCsynt}
%\vspace{-0.5cm}

	\(\yLCsynt\): tasa sintética cupón cero en moneda local a \(\tnr\) periódos en el tiempo \(\idxt\)
	
	\(\yUS\): tasa cupón cero en dólares a \(\tnr\) periódos en el tiempo \(\idxt\)
	
	\(\fwdprm\): \alert{prima futura} de tipo de cambio a \(\tnr\) periódos en el tiempo \(\idxt\) %dólar a moneda local
%	\begin{itemize}
%		\item Currency forwards (\(< 1\) year) and cross-currency swaps (\(\geq 1\) year)
%	\end{itemize}
	
	\begin{itemize}
		\item \alert{\(< 1\) Año}: Contratos adelantados de divisas (\textit{currency forwards})
%		\[(forward_{t,\tnr} - spot_{t})/\tnr\]
		\item \alert{\(\geq 1\) Año}: Permuta de divisas (\textit{cross-currency swaps}) %Fixed-for-fixed 
		%	\iftoggle{long}{\pause}{}
		\begin{itemize}
			\item Permuta de tasas de interés (\textit{interest rate swaps})
			\item Permuta de divisas variable (\textit{cross-currency basis swaps})
		\end{itemize}
	\end{itemize}
...
\frametitle{Desviaciones de la Paridad Cubierta de Tasas de Interés}
%\vspace{-0.5cm}
\[\eqCIPdevDS\]
...
\vspace{0.2cm}
	Captura:
	\begin{itemize}
		\item Avanzadas: \alert{Tasa de conveniencia} \citep*{DuImSchreger:2018JIE}
%		\item[] {\small \cite*{DuImSchreger:2018JIE}} 
		\item Emergentes: \alert{Riesgo de crédito soberano} \citep{DuSchreger:2016JoF}
%		\item[] {\small \cite{DuSchreger:2016JoF}} 
		\item Bancos: \alert{fricciones financieras} \citep*{DuTepperVerdelhan:2018} \newline
%		\item[] {\small \cite*{DuTepperVerdelhan:2018}}
	\end{itemize}

	\textbf{Este artículo}: Enfásis en \(\yLCsynt\)
\end{frame}
...
\frametitle{Base de Datos}
	\alert{15} países emergentes:
	\begin{itemize}
%		\item[] {\scriptsize BRL, COP, HUF, IDR, ILS, KRW, MYR, MXN, PEN, PHP, PLN, RUB, THB, TRY, ZAR}
		\item {\small Brasil, Colombia, Hungría, Indonesia, Israel, Corea, Malasia, México, Perú, Filipinas, Polonia, Rusia, Tailandia, Turquía, Sudáfrica}
	\end{itemize}
	%	\begin{itemize}
...
%	\iftoggle{struct}{\item<2->}{\item} 
	...
	Datos \alert{diarios}: Enero 2000 a Agosto 2021
%	\iftoggle{struct}{\item<3->}{\item}
	
	\alert{Plazos}: 0.25, 0.5, 1, 2, \ldots, 10 años
%	\iftoggle{struct}{\item<4->}{\item}
	
	Tasas \alert{sintéticas}:
	\begin{itemize}
%		\iftoggle{struct}{\item<5->}{\item}
		\item $\yUS$: \(<\) 1 año de H.15; \(\geq\) 1 año de \citet*{GSW:2007} 
%		\vspace{-0.1cm}
%		\iftoggle{struct}{\item<6->}{\item}
		\item $\fwdprm$: Bloomberg; Datastream
	\end{itemize}

\begin{textblock*}{3cm}(.87\textwidth,1.052\textheight)
	\hyperlink{YldStats}{\beamergotobutton{Estadísticas Descriptivas}}
\end{textblock*}
\end{frame}
\note{I construct nominal and synthetic yields for the 15 EMs originally studied by Du and Schreger at the daily frequency going back to 2000 up to 2021.}
...
\note{For the yields at the 3M and 6M maturities, I use data from the FR's H.15 dataset  b/c it is more robust at the short end of the YC.}
...
\section{Modelo Afín de Curvas de Tasas}
\note{Let me now describe the model used for the decomposition of the yields.}

\begin{frame}[label=ATSMsummary]
\frametitle{Descripción General del Modelo}
	
	Modelo afín estándar de curvas nominales en tiempo discreto
	\begin{itemize}
		\item Supone no incumplimiento \(\rightarrow\) Tasas \alert{sintéticas} (\(\yLCsynt\)) para emergentes 
		\item \alert{Complementado} con encuestas sobre expectativas
	\end{itemize}
	
	Intuición:
	\begin{itemize}
		\item Tasas determinadas por factores de precios \(\Xvars\)
		\item Dinámica de los factores de precios (medidas \(\Pmeasure\) y \(\Qmeasure\))
		\item Restricciones de no arbitraje garantizan coherencia % in cross section / time series of yields
%		\item Yields are affine functions of the pricing factors
	\end{itemize}
	
	\begin{textblock*}{3cm}(0.08\textwidth,1.05\textheight)
	\hyperlink{AssetPricing}{\beamergotobutton{Valuación Activos}}
	\end{textblock*}
	\begin{textblock*}{3cm}(0.27\textwidth,1.05\textheight)
		\hyperlink{SDF}{\beamergotobutton{FDE}}
	\end{textblock*}
	\begin{textblock*}{3cm}(0.36\textwidth,1.05\textheight)
		\hyperlink{BondPrices}{\beamergotobutton{Valuación Bonos}}
	\end{textblock*}
	\begin{textblock*}{3cm}(0.53\textwidth,1.05\textheight)
		\hyperlink{SvyAugModel}{\beamergotobutton{Versión Encuestas}}
	\end{textblock*}
\end{frame}
...
\frametitle{Descomposición de Tasas de Países Emergentes}
\vspace{-0.35cm}
\[\yLCnom \; \; = \; \; \yZeroQ \; + \; \CIPdev \; \; = \; \; \yZeroP \; + \; \termprm \; + \; \CIPdev \]

\vspace{0.8cm}
\begin{tabular}{ l l }
	\(\eqyZeroQ\) \(:\) & Tasas \alert{sintéticas} ajustadas \\ [15pt]
	
	\(\eqyZeroP\) \(\, :\) & Promedio de tasa corta futura esperada \\  [15pt]
	
	\(\eqTP\) \(\; :\) & Prima por plazo \\ [15pt]
	
	\(\CIPdev = \yLCnom - \yZeroQ\) \(\, :\) & Compensación por riesgo de crédito
\end{tabular}
\end{frame}
...
\note{Therefore, the TP that I propose is *clean* in the sense that it is not contaminated with credit risk as would be the case with the traditional approach. In other words, using the traditional approach for EM yields would generate a term premium contaminated with credit risk.}
...
\note{Of course, the spread is not be a perfect measure of credit risk but it is *definitely* better than ignoring it.}


\begin{frame}
\frametitle{Identificación Débil}

Tasas identifican parámetros \(\Qmeasure\) \alert{con precisión}, pero \alert{no} los parámetros \(\Pmeasure\) %están \alert{mal} identificados % , yet parameters % \{\(\XmuQ, \XPhiQ\)\} % \{\(\XmuP, \XPhiP\)\}

\begin{itemize}
	\item Tasas son persistentes y la prima por plazo no es observada
	\item Descomposiciones de tasas inestable
\end{itemize}

\textbf{Soluciones}: Encuestas, parámetros restringidos, estimadores sin sesgo

Encuestas dan descomposiciones \alert{robustas} \citep{Guimaraes:2014}
\begin{itemize}
	\item Ancla la media de largo plazo de las tasas de interés
	\item Importantes para emergentes por tamaño de muestra limitado
\end{itemize}
\end{frame}
...
\note{That is why it is desirable to have long series for the estimation.} 
...
\note{In particular, surveys are useful becase they help the model anchoring the long run mean of interest rates.}
...
\begin{textblock*}{3cm}(.87\textwidth,1.05\textheight)
	\hyperlink{YldCBP}{\beamergotobutton{Pronósticos Implícitos}}
\end{textblock*}
\end{frame}
...
\note{These calculations are done using 5Y- and 10Y-ahead forecasts.} % and b/w 5Y and 10Y ahead forecasts
...
\begin{frame}
\frametitle{Estimación del Modelo}

%	\item Use \cite*{JSZ:2011} normalization of the model %: \(\Pmeasure \perp \Qmeasure\) % likelihood functions
Parámetros estimados por MV con datos \alert{mensuales} de tasas
\begin{itemize}
\item Normalización porpuesta por \cite*{JSZ:2011}
\end{itemize}
%	\item Estimate \(\Pmeasure\) parameters by OLS
%	\item Estimate \(\Qmeasure\) parameters by MLE

Modelo con encuestas estimado con el filtro de Kalman (datos faltantes)
\begin{itemize}
\item Medida de expectativas \alert{imperfecta} \citep{KimOrphanides:2012}
\end{itemize}

Errores estándar calculados con el método delta

Factores de precios estimados con frecuencia \alert{diaria}
...
\note{Finally, I use the parameters obtained with monthly data to estimate the pricing factors at the daily frequency. This allows me to perform daily decompositions of the yields, which are needed to analyze the persistence of the effects of US MP.}
%\note{At the daily frequency there is noise in yield data that can undermine the estimation.}


\section{Descomposición de Tasas de Países Emergentes}
...
%\end{textblock*}
\begin{textblock*}{3cm}(.23\textwidth,1.08\textheight)
	\hyperlink{YldDcmp10}{\beamerreturnbutton{Descomposición Todos}}
\end{textblock*}
\begin{textblock*}{3cm}(.51\textwidth,1.08\textheight)
\hyperlink{tpCI}{\beamergotobutton{Prima por Plazo}}
\end{textblock*}
\begin{textblock*}{5cm}(.725\textwidth,1.08\textheight)
\hyperlink{crcCI}{\beamergotobutton{Compensación Riesgo Crédito}}
\end{textblock*}
\end{frame}
\note{This figure shows how the decomposition looks like over time for the 10Y *nominal* yields for HUF and MXN, which are two of the countries in the sample.}
\note{The solid blue line is the AESR. The dash-dotted red line is the TP. And the dashed purple line is the CRC.}
...
\note{In the case of MX, the CB started a tightening cycle around the 2016 US presidential election, in response to a depreciation of the currency. That move is reflected in an increase in ESR.}
...
\frametitle{Prima por Plazo e Incertidumbre Inflacionaria}

\alert{Prima por plazo} compensa por \alert{incertidumbre inflacionaria} \citep{Wright:2011} \newline
%In advanced economies related to inflation uncertainty \citep{Wright:2011} % in advanced economies

\alert{Inflación} más alta y \alert{volátil en emergentes} \citep{HaKoseOhnsorge:2019} \newline


\textbf{Pregunta}: ¿Es incertidumbre inflacionaria relevante para emergentes?

\vspace{-0.5cm}
\input{../Equations/uPanelUCSV}

\begin{itemize}
	\item \(\sigma^{\pi}_{\idxspnl}\) componente permanente, modelo UCSV \citep{StockWatson:2007}
	\item \(GDP_{\idxspnl}\) controla por el ciclo económico
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Prima por Plazo de Emergentes e Incertidumbre Inflacionaria}
...
\section{Efectos de la Política Monetaria de EUA en Tasas de Emergentes}
...
\begin{frame}
\frametitle{El Canal de la Curva de Tasas}
Política monetaria de EUA clave para ciclo financiero global \citep{Rey:2013}

%Yield curve channel
%\begin{itemize}
	Tasas de \alert{largo plazo} más influenciadas por fuerzas globales
	\begin{itemize}
		\item \alert{Autonomía monetaria} sobre la curva disminuye \citep{Obstfeld:2015}
	\end{itemize}

	Políticas \alert{no convencionales} de EUA afectan tasas de emergentes
	\begin{itemize}
		\item A largo plazo a través de la prima por plazo \citep{Turner:2014}
		\item A corto plazo a través de la tasa corta esperada \citep{Kalemli-Ozcan:2019} \vspace{-0.3cm}
		\begin{itemize}
			\item Effectos secundarios de riesgo (\textit{risk spillovers})
		\end{itemize}
	\end{itemize}
%\end{itemize}
\end{frame}
\note{The literature describes different mechanisms that I referred to as the YC channel, which can be explained as follows.}
\note{The GFCy, which refers to fluctuations in financial activity on a global scale, is mainly driven by developments in the US.} 
...
\begin{textblock*}{10cm}(67.5mm,83mm)
	\footnotesize Correlaciones Móviles
\end{textblock*}
\begin{textblock*}{5cm}(1.02\textwidth,0.6\textheight)
	\hyperlink{DYindex}{\beamergotobutton{Índice D-Y}}
\end{textblock*}
\end{frame}
...
\begin{frame}
\frametitle{¿Existe un Canal de la Curva de Tasas?}
%\begin{itemize}
...
\(\yld_{\idxspnl}\): Tasas nominales de emergentes y sus tres componentes

\(\alpha_{i}\): Efectos fijos por país

\(z^{1}_{\idxspnl}\): Descomposición de la curva de tasas de EUA \citep{KimWright:2005}

\(z^{2}_{\idxspnl}\): Factores global y nacionales
\begin{itemize}
	\item Índices VIX, EPU \citep{BakerBloomDavis:2016}, actividad global  \citep{Hamilton:2019}
	\item Tasa de política, inflación, desempleo, tipo de cambio %(LC per USD)
\end{itemize}
%\end{itemize}
\end{frame}
...
\begin{textblock*}{5cm}(1.07\textwidth,0.23\textheight)
	\hyperlink{Drivers10Y}{\beamergotobutton{10A}}
\end{textblock*}
\begin{textblock*}{5cm}(1.07\textwidth,0.59\textheight)
	\hyperlink{Drivers2Y}{\beamergotobutton{2A}}
\end{textblock*}
\end{frame}
...
\note{First, notice that the response of ESR to the domestic policy rate is stronger at the short end than at the long end. Also, the ESR is associated with its US counterpart only at the long end but not at the short end ----> This evidence shows that the monetary autonomy in EMs is stronger at the short end than at the long end.}

\note{The second point is that the response of the term premia in EMs to the US TP increases with maturity. Also, the term premia in EMs is positively associated with the VIX only at the long end ----> This shows that the GFCy (represented here by the US TP and the VIX) is more relevant for LT yields than for ST ones.}

\note{The last point is that there are risk spillovers to the ESR in EMs from the US TP. Those risk spillovers are stronger at the short end than at the long end as suggested by Kalemli-Ozkan.} % but not from the VIX
...
\begin{frame}
\frametitle{Sorpresas en la Política Monetaria de EUA}

Cambio en precio de activos en ventanas de 2hr de reuniones del FOMC
%Surprises:
	\begin{itemize}
		\item \alert{Nivel}: cambio en futuros de tasa de fondos federales \citep{Kuttner:2001}
		\item \alert{Trayectoria}: residual de cambio en 8vo. futuro de Eurodólar sobre sorpresa de nivel \citep{GSS:2005a}
		\item \alert{Compras de Activos}: residual de cambio en futuros del bonos de EUA a 10 años sobre sorpresas de nivel y trayectoria \citep{Swanson:2018} % target and forward guidance surprises starting in 2009
	\end{itemize}
\begin{center}
\input{../Figures/MPS/PMEUtiempos}
\end{center}
%\begin{center}
...
\begin{frame}
\frametitle{Medición de Efectos sobre Tasas de Países Emergentes}
%\vspace{-1cm}
Proyecciones locales de panel:

\vspace{-0.4cm}
\input{../Equations/uPanelLP}	% \ref{eq:nPanelLP}
%\vspace{-0.7cm}
\begin{itemize}
\item \(\yld_{\idxspnl}\): 10A and 2A \alert{tasas} nominales de emergentes y sus \alert{componentes}
\item \(\idxh = 0, 1, \ldots, 45\) días
\item \(\alpha_{\idxh,\idxi}\): Efectos fijos por país
\item \(\epsilon^{j}_{\idxt}\): \alert{tres} tipos de \alert{sorpresas} de política monetaria
\item \(\fx_{\idxspnllag}\): rezago de un día en el tipo de cambio
\end{itemize}
\end{frame}
...
\hyperlink{TargetUS}{\beamergotobutton{EUA}}
\end{textblock*}
\end{frame}
...
\end{textblock*}
\begin{textblock*}{5cm}(1.07\textwidth,0.65\textheight)
	\hyperlink{FGUSpre}{\beamergotobutton{EUA}}
\end{textblock*}
\end{frame}
...
\frametitle{Efectos de una Relajación de Trayectoria en Tasas de EM: Post-CFG}
\begin{figure}[!htbp]
\begin{center} % trim removes: left, down, right, top
...
\end{textblock*}
\begin{textblock*}{5cm}(1.07\textwidth,0.65\textheight)
\hyperlink{FGUSpost}{\beamergotobutton{EUA}}
\end{textblock*}
\end{frame}
...
\end{textblock*}
\begin{textblock*}{5cm}(1.07\textwidth,0.65\textheight)
\hyperlink{LSAPUS}{\beamergotobutton{EUA}}
\end{textblock*}
\end{frame}
...
Descomposición de \textbf{tres} partes de las tasas soberanas de emergentes
\begin{itemize}
\item Tasa corta esperada promedio \item Prima por plazo \item Compensación por \alert{riesgo de crédito}
\end{itemize}

Política monetaria de EUA \textbf{afecta} las tasas soberanas de emergentes
\begin{enumerate}
	\item Efectos son económicamente \alert{significativos} pero con \alert{rezago}
	\item Reevaluación de expectativas de la tasa de política y de \alert{riesgos}
	\item Evidencia de un \alert{canal de la curva de tasas} desde 2008
\end{enumerate}
\end{frame}
...
\note{I am happy to answer any other question.}
...
\frametitle{Riesgo Crediticio en los Bonos Emitidos en Moneda Local}
\begin{center}
...
\begin{textblock*}{5cm}(0.87\textwidth,1.06\textheight)
	\hyperlink{SovereignDefaults}{\beamerreturnbutton{Incumplimientos Soberanos}}
\end{textblock*}
\end{frame}
\note{Few EM have a credit rating of A and more than 30 have a rating below investment grade of BBB.}
...
\begin{textblock*}{3cm}(1.02\textwidth,.18\textheight)
	\hyperlink{YldData}{\beamerreturnbutton{Base Datos}}
\end{textblock*}
\end{frame}
...
\frametitle{Valuación de Activos}
Bajo no arbitraje \(\rightarrow\) \(\exists\) un factor de descuento estocástico \(\SDF > 0\)
\newline

\(\SDF\) valúa todos los bonos nominales bajo la medida de probabilidad \(\Pmeasure\)
\input{../Equations/uPzeroP}
\vspace{1pt}

\(\SDF\) \(\rightarrow\) \(\exists\) una medida neutral al riesgo \(\Qmeasure\) definida como % theoretical risk-neutral pricing
\input{../Equations/uPzeroQ}

\begin{textblock*}{3cm}(1\textwidth,1.05\textheight)
	\hyperlink{ATSMsummary}{\beamerreturnbutton{Descripción}}
\end{textblock*}
\end{frame}
...
\frametitle{Factor de Descuento Estocástico}
...
\frametitle{Valuación de Bonos}
Factores de precios bajo la medida \(\Pmeasure\) % physical
\input{../Equations/uXvarsP}

Precios de los bonos % is an exponentially affine function of the pricing factors
...
Factores de precios bajo la medida \(\Qmeasure\) % risk-neutral
...
\frametitle{Modelo Aumentado con Encuestas}

Tasa corta promedio esperada % under \(\Pmeasure\)
\input{../Equations/uYaffineE}
\vspace{3pt}

Tasa futura de \(\tnr\) a \(\tnrfwd\) periodos adelante
...
\begin{textblock*}{15cm}(2.5mm,3mm)
	\textbf{10 Años}
\end{textblock*}
\begin{textblock*}{5cm}(0.99\textwidth,1.05\textheight)
	\hyperlink{SCBP}{\beamerreturnbutton{Encuestas}}
\end{textblock*}
\end{frame}
...
\begin{textblock*}{5cm}(0.92\textwidth,1.07\textheight)
	\hyperlink{YldDcmp2}{\beamerreturnbutton{Descomposición HUF-MXN}}
\end{textblock*}
\end{frame}
...
\end{textblock*}
\begin{textblock*}{5cm}(0.97\textwidth,1.05\textheight)
\hyperlink{YldDcmp10}{\beamerreturnbutton{Descomposición}}
\end{textblock*}
\end{frame}
...
\begin{textblock*}{5cm}(1.01\textwidth,0.55\textheight)
	\hyperlink{RollingCorr}{\beamergotobutton{Corr. Móviles}}
\end{textblock*}
\end{frame}
...
\begin{textblock*}{5cm}(1.04\textwidth,0.09\textheight)
	\hyperlink{Drivers10Y2Y}{\beamerreturnbutton{10A \& 2A}}
\end{textblock*}
\end{frame}
