...
\gototoc
\begin{todolist}
	\item Add usyc. Compare response of usyc vs results in Brooks, Kratz and Lusting. Results of usyc across panels should be similar to usyc alone. Explain movements in ysynt due to rho and usyc.
	\item Standard errors robust to heteroskedasticity, autocorrelation and cross-sectionally dependent. Number of lags = 4.
	\item Analysis on spillovers: Responses of EM components to US MPS (regressions as in Curcuru or panel local projections as in ACDM); lags in response; asymmetric effects on different parts of the YC; changes in components around QE-TT announcements.
...
\begin{todolist}
	...
	\item Create a structure for US data: Vix, USTP, USYP, SVEY, EPU, FFR, USMPS.
	\item Estimate Guimaraes w/ daily data for US and compare against KW.
	\item Check whether functions in post-estimation analysis need to be updated after the bsl field added to the structure.
	...
	\item Include TIC data in dataset.
...
\gototoc
\begin{todolist}
	\item Generate dta file with new US MPS (excluding Sept 2001 meeting).
	...
	\item Re-estimate parameters after 0.25 and 0.5 in matsout, RHO in S.
