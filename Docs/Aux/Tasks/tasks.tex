...
\begin{todolist}
	...
	\begin{todolist}
		...
		\item Re-estimate the models for HUF (0.16), IDR (0.39), ILS (0.10), MYR (0.17), PHP (0.37), THB (0.135), TRY (0.12), ZAR (0.07). RMSE in parenthesis.
		\item After estimation: redefine baseline estimations (for ILS and ZAR), assess_fit and add_vars.
		\item Compare results against previous ones.
		\item Stock and Watson inflation decomposition. Compare trend component vs HP filter trend for ILS and ZAR.
		\item Link to recession shaded areas: https://www.mathworks.com/matlabcentral/answers/243194-shade-an-area-in-a-plot-between-two-y-values
	\end{todolist}
	...
	\begin{todolist}
		...
		\item Ensure all yields generated are in decimals (daily2dymy makes yields in decimals, append_svys2ylds makes surveys in decimals, thus yields in all versions of model are in decimals).
		...
		\item SD of permanent component from Stock-Watson.
	...
	\begin{todolist}
		...
		\item Redefine regions as in Hofmann, Shim and Shin and see LPs. If not regional, it might be groups of countries who respond similarly; use individual LPs to form those groups. Unlike AEs, there is a lot of heterogeneity in the response of EMs.
	...
	\begin{todolist}
		...
		\item PCs for: real rates (global RR), inflation (global inflation factor), inflation and growth expectations (common trends in interest rates). Use the code from trend_inflation to get a common table of the variable of interest.
