...
\begin{todolist}
	...
	\begin{todolist}
		...
		\item Update figures.
		\item Include in ts_pca PCs for: real rates (global RR), inflation (global inflation factor 81\%), inflation expectations (PC1: 51\%, PC2: 24\%) and growth (common trends in interest rates), inflation volatility S-W (93\%), inflation trend S-W (81\%), inflation trend HP (93\%).
		\item Use global activity indexes for GFCy and C-P global factor.
		\item Compare yP vs scbp for 1Y (not used in estimation).
		\item Update emtimetable to emaetimetable and include option to generate TT for AE (to be used by ts_plots for AEs). Update call for it in trend_inflation.
	...
	\begin{todolist}
		...
		\item Check that variables w/ and w/o header are correctly stored.
		\item Ensure all tenors are in months: TT_kw, TT_rr.
		\item Identify variables that are not in decimals but in percent: MPS, TT_kw, TT_rr.
		\item Ensure scbp for 1Y is included to verify yP vs scbp for 1Y.
		\item Include daily 3 PCs and estimated pricing factors for each country. Maybe in add_vars.
		\item Include global PC and non-US common factor. common PC for EM and AE, non-US commons for EM and AE. Compare the common PC in EM and AE b/w them and to US YC components, global activity indexes, local conditions.
		\item Re-estimate flexible model mssf for: HUF, IDR, ILS, MYR, THB, TRY, ZAR.
		\item Re-estimate  flexible model mssf for: PHP.
	\end{todolist}
	...
	\begin{todolist}
		...
		\item If not regional response, it might be groups of countries who respond similarly; use individual LPs to form those groups. Unlike AEs, there is a lot of heterogeneity in the response of EMs.
		\item Substitute . by zeros in US MPS.
		\item Try lag of depvar as in Hofmann, Shim and Shin and see LPs.
		\item Express all variables in basis points.
	...
	\begin{todolist}
		...
		\item Variable of financial openness (Chinn-Ito index). Linear interpolation for monthly frequency.
	\end{todolist}
...
\gototoc
	...
	\begin{todolist}
		...
		\item[\wontdo] Re-estimate baseline models (mnsb, mssb) for BRL (0.27), IDR (0.47), MYR THB, TRY, ZAR. Run code for all those countries takes time and is prone to freeze. So I better estimate individually and only the mssb version.
