...
\gototoc
\begin{todolist}
	\item Compare monthly w/ daily fit and decompositions.
	\item Include path and tp shocks in TT_mps.
	\item Include EPU US in dataset.
	\item Include global activity index (Hamilton and Kilian). (https://sites.google.com/site/lkilian2019/research/data-sets) for GFCy and C-P global factor.
	\item Include country-specific data for EMs in dataset: STX, surveys, real rates.
	\item Analysis on spillovers: Responses of EM components to US MPS (regressions as in Curcuru or panel local projections as in ACDM); lags in response; asymmetric effects on different parts of the YC; changes in components around QE-TT announcements; local events.
