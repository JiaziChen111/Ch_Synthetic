...
\begin{todolist}
	...
	\item[\done] Is there a risk channel for monetary policy spillovers? Are LT rates more highly correlated than ST rates? Policy rate disconnect for ST and/or LT? US spillovers work through changes in risk premia? Expected rate in EMs react to changes in USTP?
	\item[\done] International comparisons of expectations (yE). Subtract inflation expectations and see whether r* differs. Is there a single world real interest rate? Is the real interest rate the same for all EMs? Plot min-max intervals to see spread like Kalemli-Ozcan for policy rates.
	...
	\item[\done] High inflation uncertainty leads to high TP?
	\item[\done] GFCy is different at the short than at the long end of the curve. Obstfeld (2015)
\end{todolist}
...
\begin{todolist}
	...
	\begin{todolist}
		\item Explanation of bias in VAR is b/c state variable is very persistent.
		\item Reason to use CRSP: GSW doesn't target ST.
		\item Reference to Chernov et al. and to SP credit ratings for EM instead of R-R 2011 in section 2.1. 
		...
		\item Evidence from DY index on different maturities show evidence of market segmentation, which allows to explain effects of LSAP on TP (W \& W 2020).
		\item Evidence that CRC is more idiosyncratic from drivers and DY index.
		\item TS of TP DY index.
		\item Save figures to be used in pdf.
		\item Read literature and prepare litreview file. Discuss insights of yield decompositions. Wright 2011 description. Adrian 2017 description. Sack 2008 description. Read KW 2005 explanations for declining TP \(<->\) Episodes of negative TP per country \(<->\) Portfolio inflows \(<->\) Taper tantrum. LCCS properties in D-S. Hoffman-Shim-Shin. Financial integration. Russia case discussed in D-S: can the 3-part decomposition shed light? % Correlation w/ inflation, changing pi*: do surveys help w/ this; is high inflation a risk factor. 
...
\begin{todolist}
	...
	\item Correct Hessian in asyvarhat considering that it seems to work for AEs, check whether excluding surveys corrects the issue.
\end{todolist}
...
\begin{todolist}
	...
	\item MPS from ECB. Which MPS matter more for EMs, Fed or ECB? Does it varies by region (some countries respond more to ECB than to the Fed)? Beyond scope of current paper.
...
\gototoc
	\begin{todolist}
	\item[\done] Feeling of data. Summary statistics of yield (Guimaraes) and survey data (inflation, GDP, short rate). 
	\item[\done] Term structure of TP. Averages per country.
	\item[\done] Correlation of BRP and TPnom. Averages of components (relative importance) per country. Before and after the GFC.
	\item[\wontdo] t-test for LCCS = 0 and TPsynt = 0. LCCS = 0 is equivalent to test TPnom = TPsynt.
	\item[\wontdo] Average and std of real interest rate to see if on average is the same across EMs and how much variability it has compared to nominal and synthetic yields. Correlation b/w real rates of EMs.
	\item[\done] Compare ssb_yP vs CBP surveys. Similar to comparing against model-free TPsvy. Comment this in footnote.
	\item[\done] Compare TPs w/ empirical measures as in Curcuru et al. 2018. Compare against survey-based TP, and measures of risk and uncertainty.
	\item[\done] Fit. RMSE. Averages per country. Nominal AEs and synthetic EMs (incl. ILS and ZAR).
	\item[\done] Notes for tables.
	\item[\done] Slides.
	\begin{todolist}
		\item[\done] Figures. Labels, titles, notes. Clearly state maturity of the yield reported.
		\item[\done] Tables and figures. Add circles/rectangles or highlight numbers w/ color.
	\end{todolist}
	\item[\wontdo] Fisher equation rather than approximation for US real rate and EM nominal rate. Not clear how to do it once the correction for the 'real' TP is included.
	\item[\wontdo] Start sample for Russia 1 or 2 months after the current initial date. Forget to include it in the last update.
	\item[\wontdo] Compare scbp obtained w/ SPF data vs survey data from other AEs. Time consuming; can reference to international evidence based on Laubach and Williams.
	\item[\done] Make reference to Chernov, Schmidt, Song about credit risk in US YC.
	\item[\done] Recalculate implied forecasts of EM short rates considering currency risk premia.
	\item[\done] Update LPs using absphi. Countries w/ truncated phi: COP (minimal), HUF (only early in sample), IDR (-51 bps), ILS (GR), KRW (minimal), MXN (ave -15 bps), MYR (minimal) PEN (-31 bps), PHP (-66 bps), PLN (only early in sample), RUB (minimal), THB (GR), TRY (since 2014, avg -59 bps), ZAR (ave -33 bps). Avg = -43 bps.
	\item[\done] Risks of global investors when investing in LC bonds: risk of a LC depreciation, risk of default.
	\item[\done] Truncate CRC at 0 in dcmp figure.
	\item[\wontdo] Update label for Avg Exp SR in figures.
	\item[\done] Include figure for yield decomposition, reflecting (in terms of sizes per maturity) averages of components.
	\item[\done] Include cashflow diagram to construct synthetic yields.
	\item Tables. Include summary tables. Update notes. 
	\item[\done] Write down equation for CRC.
	\item[\done] Review explanation in HSS on Nickell bias.
	\item[\done] Review references.
	\item[\done] Review notes of all tables and figures (eg. 90\% CI) in main text and appendix.
	\item[\done] Assumption: same investors in LCNOM and LCSYNT. 
	\item[\done] Make references in the main text to all things in appendices.
	\item[\done] Wording.
	\begin{todolist}
		\item[\done] Remove that only FOMC meetings but leave that no minutes nor speeches.
		\item[\done] Footnote for differences in SPF-SPD-SMP.
		\item[\done] Reason to use CRSP: GSW doesn't target ST, follow Golinski \& Spencer.
		\item[\done] Comparison of real rates from surveys and TIPS.
		\item[\done] Reason for day shift.
		\item[\done] Write equation for inflation uncertainty part.
	\end{todolist}
	\item[\done] Technical parts.
	\begin{todolist}
		\item[\done] Drivers before vs after GFC. Influence of US YC components and the Vix increased post-GFC. Unemployment larger role for TP pre-GFC.
		\item[\done] Include UCSV std in drivers. Negative sign after including components of US YC.
		\item[\done] Adjust cutoff date for LSAP, Oct 2008 (rather than Jan 2009) according to Rogers et al (2018). Update: summary table of US MPS, LPs (all b/c shocks all at once, including path pre/post).
		\item[\done] Rolling correlation in addition to DY index.
		\item[\done] Check difference between surveys among economists and surveys among traders. A few basis points b/w SPF and SPD, and b/w SPF and SMP.
		\item[\done] Verify that fitting error is left in CRC and is not big to change to a 4-way decomposition.
		\item[\done] Check dates in USMPS file against dates in Rogers et al. (2014) to verify it only includes FOMC meetings (no speeches). Only FOMC meetings.
		\item[\done] Check which parameters seem to be precisely estimated. B/c real and imaginary parts, the latter are ignored when plotting so is akin to SE = 0.
		\item[\done] Use linewidth to make lines in Matlab bigger.
		\item[\done] LPs. Update equation in levels.
		\item[\done] LPs: update figures in levels (including path pre/post).
	\end{todolist}
	\item[\done] In drivers tables (1, 2, 5 10 Y) indicate that USYP and USTP have same maturity as dependent variables. Indicated in notes.
	\item[\wontdo] Repeat analysis using dataset of Wu (competes against GSW) to see how sensitive the results are to using GSW. GSW fitting error of 3 bps, improvement of Wu is in short end for which GSW doesn't fit well.
	\item[\done] Robustness checks: (1) compare scbp obtained w/ SPF data vs TIPS. 
	\item[\wontdo] Re-estimate models using scbp constructed w/ TIPS data rather than SPF data. TIPS data is not the benchmark.
	\item[\done] Update slides for presentation in brown bag seminar.
	\item[\wontdo] Correlation of YC components with the 3 PCs and estimated pricing factors. PCs are not relevant for ATSM, only used for estimation.
	\item[\wontdo] LPs for effects of US MPS on the 3 pricing factors of EMs.
	\item[\wontdo] Skim through Warnock papers on TIC data. No longer need to include TIC data.
	\item[\wontdo] Include TIC data for each country (inflows and outflows) in dataset.
	\item[\wontdo] Variable of financial openness (Chinn-Ito index). Linear interpolation.
	\item[\done] Annex.
	\begin{todolist}
		\item[\done] Trend inflation and figures for ILS-ZAR.
		\item[\done] Figures: LPs for US, LPs for AEs, LPs for EMs regions, LPs for AEs blocks.
		\item[\wontdo] How the size of the rolling window influence the DY index for EMs? I will report rolling correlation and DY index for support, so no need for robustness of DY index.
	\end{todolist}
	\item[\done] Future work in conclusions: future work on decomposing further the yields (real parts, CR compensation), inform theoretical models for pricing sovereign defaultable bonds, effects of domestic MP, effects of other AE CB.
	\item[\done] LPs: Use target, forward guidance and asset purchase instead of target, path, LSAP.
	\item[\done] Effects on YC slope in section 5.
	\item[\done] Reference to DK SE for LPs (text and footnotes). Reference to Pesaran test. Indicate how default value for the lags is chosen. e.g. in Brooks et al.: `We plot 2-standard-error bands around theIR. HAC standard errors computed with bandwidth 2 fork<=50.'
	\item[\done] Reframe paper in terms of Obstfeld-Ozcan debate. Highlight that ignoring credit risk alters the assessment of the debate (instead of framing the whole paper about the implication of ignoring credit risk).
	\item[\done] Importance of TP: how resources are allocated in the economy, transmission of MP.
	\item[\done] LPs vs VAR to compute IRFs.
	\item[\done] Reference to DK se for tables. Reference to Pesaran test.
	\item[\done] Explain that phi = ynom - yfitted (not ynom-ysynt as before).
	\item[\done] Explain why not showing AE decompositions.
	\item[\done] Follow Sahm's recommendations.
	\item[\done] Read Cochrane shared by Moffitt.
