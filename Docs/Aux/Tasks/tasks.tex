...
\gototoc
\begin{todolist}
	\item Update code for LPs:
	\begin{todolist}
		\item Correct lag of dependent variable for LPs.
		\item LPs for 3 shocks for the respective sample periods.
		\item Create blocks for AEs as regions for EMs.
		\item LPs for regions of EM and blocks of AE.
		\item DK standard errors robust to heteroskedasticity, autocorrelation and cross-sectionally dependent. Number of lags = 4. Last b/c they will take more time.
	\end{todolist}
	\item TP regressions. 
	\begin{todolist}
		\item Drivers (monthly): Panel regressions for TP, LCCS, yE for AEs and EMs. Currently, no macro data for AEs.
	\end{todolist}
	\item Correlations using monthly data (EPU, surveys).
	...
	\begin{todolist}
		\item Update ts_analysis file to generate dataset.
		\item Include KW decomposition for USYC.
