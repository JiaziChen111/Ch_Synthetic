...
\begin{todolist}
	...
	\begin{todolist}
		...
		\item Restrict sample for RUB to start in July 2009. Explain in paper.
	\end{todolist}
	\item Re-estimate parameters using implied forecasts for CBP.
	\begin{todolist}
		\item Check if there is no issues in estimation due to shorter history for 5Y US real rate relative to 10Y.
	\end{todolist}
	...
	\begin{todolist}
		...
		\item Update references to figures due to changes in names (4 - fit, 6 yP vs scbp).
	...
	\begin{todolist}
		...
		\item Include surveys for ALL tenors, currently construct_panel makes no exception for matsout.
		\item Ensure all tenors in months and all yields in decimals.
	\end{todolist}
