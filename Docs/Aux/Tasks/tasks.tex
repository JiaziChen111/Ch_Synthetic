...
\begin{todolist}	
	\item Paper.
	\begin{todolist}
		\item Effects on YC slope in section 5.
		\item In drivers tables (1, 2, 5 10 Y) indicate that USYP and USTP have same maturity as dependent variables.
		...
		\item Make references to all things in appendices.
		\item Review notes of all tables and figures (eg. 90\% CI) in main text and appendix.
		\item LPs: Use target, forward guidance and asset purchase instead of target, path, LSAP.
		\item Save figures to be used in pdf.
		\item Future work in conclusions: future work on decomposing further the yields (real parts, CR compensation), inform theoretical models for pricing sovereign defaultable bonds, effects of domestic MP, effects of other AE CB.
...
\gototoc
\begin{todolist}
	\item Adjust threshold for path and LSAP pre- and post GFC, cutoff data October 2008 according to Rogers et al (2018). Maybe activate target after 2015. Update: summary table of US MPS, LPs (all b/c all shock at once, path pre/post).
	...
	\item Drivers before vs after GFC.
